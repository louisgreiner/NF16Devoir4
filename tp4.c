...
// Renvoie le noeud dont la date (= clé) est la plus petite de l'arbre après celle entrée en paramètre
...
void supprimerNoeud(T_ABR** abr, char* date)
{
    ...
    if (comparerDates(date, (*abr)->date) > 0) {
        supprimerNoeud(&((*abr)->fils_droit), date); // ou supprimerNoeud(&(*abr)->fils_droit, date);
        return;
    }
    // sinon
    if (comparerDates(date, (*abr)->date) < 0) {
        supprimerNoeud(&((*abr)->fils_gauche), date); // ou supprimerNoeud(&(*abr)->fils_gauche, date);
        return;
    }

    if ((*abr)->fils_gauche == NULL) {
        free((*abr)->liste_vaccins);
        free((*abr)->date);
        ...
        *abr = fils_droit;
        ...
        free((*abr)->liste_vaccins);
        free((*abr)->date);     //je sais pas pourquoi ça plante là, la réponse demain... euh aujourd'hui du coup
        ...
        *abr = fils_gauche;
        ...
        strcpy((*abr)->liste_vaccins, (*succ)->liste_vaccins);
